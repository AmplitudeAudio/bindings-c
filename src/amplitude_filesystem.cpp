...
public:
    ...
    {
        const auto file = _v_table->open_file(_user_data, path.c_str(), static_cast<am_file_open_mode>(mode));
        ...
        if (file.type == am_file_type_package_item)
            return std::shared_ptr<PackageItemFile>(static_cast<PackageItemFile*>(file.handle));

#if AM_PLATFORM_ANDROID
        if (file.type == am_file_type_android_asset)
            return std::shared_ptr<AssetManagerFile>(static_cast<AssetManagerFile*>(file.handle));
...
am_file_handle am_file_create(const am_file_config* config)
{
    if (config->type == am_file_type_custom)
        return { am_file_type_custom, ampoolnew(eMemoryPoolKind_IO, CFile, config->v_table, config->user_data) };
...
void am_file_destroy(am_file_handle handle)
{
    ...
    {
        ampooldelete(eMemoryPoolKind_IO, CFile, static_cast<CFile*>(handle.handle));
    }
    else if (handle.type == am_file_type_disk)
    {
        ampooldelete(eMemoryPoolKind_IO, DiskFile, static_cast<DiskFile*>(handle.handle));
    }
    else if (handle.type == am_file_type_memory)
    {
        ampooldelete(eMemoryPoolKind_IO, MemoryFile, static_cast<MemoryFile*>(handle.handle));
    }
}
...
{
    static_cast<File*>(file.handle)->Seek(offset, static_cast<eFileSeekOrigin>(origin));
}
...
am_filesystem_handle am_filesystem_create(const am_filesystem_config* config)
{
    if (config->type == am_filesystem_type_custom)
        return { am_filesystem_type_custom, ampoolnew(eMemoryPoolKind_IO, CFileSystem, config->v_table, config->user_data) };

    if (config->type == am_filesystem_type_disk)
        return { am_filesystem_type_disk, ampoolnew(eMemoryPoolKind_IO, DiskFileSystem) };

    if (config->type == am_filesystem_type_package)
        return { am_filesystem_type_package, ampoolnew(eMemoryPoolKind_IO, PackageFileSystem) };

#if AM_PLATFORM_ANDROID
    if (config->type == am_filesystem_type_android)
        return { am_filesystem_type_android, ampoolnew(eMemoryPoolKind_IO, AssetManagerFileSystem) };
#elif AM_PLATFORM_IOS
    if (config->type == am_filesystem_type_ios)
        return { am_filesystem_type_ios, ampoolnew(eMemoryPoolKind_IO, NSFileSystem) };
#endif
...
void am_filesystem_destroy(am_filesystem_handle filesystem)
{
    ...
    {
        ampooldelete(eMemoryPoolKind_IO, CFileSystem, static_cast<CFileSystem*>(filesystem.handle));
    }
    else if (filesystem.type == am_filesystem_type_disk)
    {
        ampooldelete(eMemoryPoolKind_IO, DiskFileSystem, static_cast<DiskFileSystem*>(filesystem.handle));
    }
    else if (filesystem.type == am_filesystem_type_package)
    {
        ampooldelete(eMemoryPoolKind_IO, PackageFileSystem, static_cast<PackageFileSystem*>(filesystem.handle));
    }
#if AM_PLATFORM_ANDROID
    else if (filesystem.type == am_filesystem_type_android)
    {
        ampooldelete(eMemoryPoolKind_IO, AssetManagerFileSystem, static_cast<AssetManagerFileSystem*>(filesystem.handle));
    }
#elif AM_PLATFORM_IOS
    else if (filesystem.type == am_filesystem_type_ios)
    {
        ampooldelete(eMemoryPoolKind_IO, NSFileSystem, static_cast<NSFileSystem*>(filesystem.handle));
    }
#endif
}
...
{
    return am_allocate_osstring(static_cast<FileSystem*>(filesystem.handle)->GetBasePath());
}

const am_oschar* am_filesystem_resolve_path(am_filesystem_handle filesystem, const am_oschar* path)
{
    return am_allocate_osstring(static_cast<FileSystem*>(filesystem.handle)->ResolvePath(path));
}
...
const am_oschar* am_filesystem_join(am_filesystem_handle filesystem, const am_oschar** parts, am_size count)
{
    ...
    return am_allocate_osstring(static_cast<FileSystem*>(filesystem.handle)->Join(cpp_parts));
}
...
{
    if (filesystem.type != am_filesystem_type_package || internal->type == am_filesystem_type_package)
        return;

    auto* fs = static_cast<PackageFileSystem*>(filesystem.handle);

    if (internal->type == am_filesystem_type_custom)
        fs->SetPlatformFileSystem<CFileSystem>(internal->v_table, internal->user_data);
