...
public:
  ...
  AmVoidPtr Malign(eMemoryPoolKind pool, AmSize size,
                   AmUInt32 alignment) override {
    return _v_table->malign(static_cast<am_memory_pool_kind>(pool), size,
                            alignment);
  }

  AmVoidPtr Realign(eMemoryPoolKind pool, AmVoidPtr ptr, AmSize size,
                    AmUInt32 alignment) override {
    return _v_table->realign(static_cast<am_memory_pool_kind>(pool), ptr, size,
                             alignment);
  }
...
void am_memory_manager_deinitialize() { MemoryManager::Deinitialize(); }

am_bool am_memory_manager_is_initialized() {
  return BOOL_TO_AM_BOOL(MemoryManager::IsInitialized());
}

am_voidptr am_memory_manager_malloc(am_memory_pool_kind pool, am_size size,
                                    const char *file, am_uint32 line) {
  return amMemory->Malloc(static_cast<eMemoryPoolKind>(pool), size, file, line);
}

am_voidptr am_memory_manager_malign(am_memory_pool_kind pool, am_size size,
                                    am_uint32 alignment, const char *file,
                                    am_uint32 line) {
  return amMemory->Malign(static_cast<eMemoryPoolKind>(pool), size, alignment,
                          file, line);
}

am_voidptr am_memory_manager_realloc(am_memory_pool_kind pool,
                                     am_voidptr address, am_size size,
                                     const char *file, am_uint32 line) {
  return amMemory->Realloc(static_cast<eMemoryPoolKind>(pool), address, size,
                           file, line);
}

am_voidptr am_memory_manager_realign(am_memory_pool_kind pool,
                                     am_voidptr address, am_size size,
                                     am_uint32 alignment, const char *file,
                                     am_uint32 line) {
  return amMemory->Realign(static_cast<eMemoryPoolKind>(pool), address, size,
                           alignment, file, line);
}
...
am_size am_memory_manager_size_of(am_memory_pool_kind pool,
                                  const am_voidptr address) {
  return amMemory->SizeOf(static_cast<eMemoryPoolKind>(pool), address);
}

void am_memory_free_str(const char *str) { am_free_string(str); }

#ifndef AM_NO_MEMORY_STATS
const char *am_memory_manager_get_memory_pool_name(am_memory_pool_kind pool) {
  return am_allocate_string(
      MemoryManager::GetMemoryPoolName(static_cast<eMemoryPoolKind>(pool))
          .c_str());
}

am_memory_pool_stats_handle
